...
- One and only one "actor" to One or Many "film_actor"

2. What does the blue diamond next to the "last_update" column on the "inventory" table represent?

- The Blue Diamond indicates that "last_update" is NOT NULL simple attribute.

3. How many foreign keys does the "payments" table have? How can you tell?

- There are 3 foreign keys to payments. The red icons on the side indicate a foreign key. 2 Filled Red diamonds indicate a NOT NULL foreign key and one unfilled red diamond indicating the foreign key can be NULL. Also hovering over the entity on mySQL highlights the foreign key relationships in green.
...
2. Copy the screenshot to this directory and upload it (git push) along with this README
   ![mySQL ER Diagram](/img/wk6_er_diagram.jpg "er_diagram")
